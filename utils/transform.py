...
def transform_data(data, exchange_rate):
    ...
    df['Price_in_dollars'] = df['Price'].str.replace(r"$", "", regex=False).astype(float)
    df['Rating'] = df['Rating'].replace("Rating: ⭐", "", regex=True).str.strip()
    df['Rating'] = df['Rating'].replace("/ 5", "", regex=True).str.strip().astype(float)
    df['Number Color'] = df['Number Color'].replace("Colors", "", regex=True).str.strip().astype(int)
    df['Size'] = df['Size'].replace("Size:", "", regex=True).str.strip().astype('string')
    df['Gender'] = df['Gender'].replace("Gender:", "", regex=True).str.strip().astype('string')
